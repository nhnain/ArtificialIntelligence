...
class HeuristicGrade(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        points = 0
        base_time = 30
        points = 16 + int((base_time - spend_time)/10)
        grades.addPoints(points)        
        grades.addMessage('Based time: %0.2f.' % base_time)
        grades.addMessage('Your elapsed time: %0.2f. You got 1 point deduced for every 10 secs more.' % spend_time)
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        handle.write('cost: "%d"\n' % len(solution))
        handle.write('path: """\n%s\n"""\n' % wrap_solution(solution))
        handle.write('thresholds: "2000 1950 1900 1850 1800 1700 1600 1550 1500 1450 1400 1350"\n')
        handle.close()
        return True
